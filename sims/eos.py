...
def eos_al_func(save_every, threshold, time_count, N_init, path, N, kernel, iter_num,
                density_min, density_max, xlo, xhi, exp_resolution, ar=1.61, zoom=3.5):
    var_list = []
    ...
    for time_step in range(time_count):
        ...
        if(time_step % save_every == 0):
            ...
            ax.text(.2, 0.8, f'Timestep = {time_step+1}', ha='center', va='center',
                    transform=ax.transAxes, fontsize=7, bbox=dict(facecolor='red', alpha=0.5))
